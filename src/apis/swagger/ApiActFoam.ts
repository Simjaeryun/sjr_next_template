...
export class CalculationFoamApi {
	...
	async getListMarid(pid: string, uid: string, marid: string): Promise<any[]> {
		const { data } = await this.axios({
			...
			url:
				ApiUrl.baseUrl +
				ApiUrl.CALCULATION_GET_MARID_FOAM.replace("{pid}", pid)
					.replace("{uid}", uid)
					.replace("{marid}", marid),
		});
		return data.data;
	...
	async getListMaridDelyn(param: any): Promise<any[]> {
		const { data } = await this.axios({
			...
			url:
				ApiUrl.baseUrl +
				ApiUrl.CALCULATION_GET_MARID_FOAM.replace("{pid}", param.pid)
	...
	async load(pid: string, uid: string, marid: string): Promise<any[]> {
		const { data } = await this.axios({
			...
			url:
				ApiUrl.baseUrl +
				ApiUrl.CALCULATION_FORM_GET_LOAD.replace("{pid}", pid)
					.replace("{uid}", uid)
					.replace("{marid}", marid),
		});
		return data.data;
	...
	async save(param: any): Promise<any[]> {
		...
		const { data } = await this.axios({
			method: "POST",
			url: ApiUrl.baseUrl + ApiUrl.CALCULATION_POST_SAVE_FOAM,
			data: param,
			timeout: 700000,
		});
		return data;
	...
	async issue(param: any): Promise<any[]> {
		...
		const { data } = await this.axios({
			...
			url:
				ApiUrl.baseUrl +
				ApiUrl.CALCULATION_POST_ISSUE_FOAM.replace("{pid}", param.pid),
			params: {
	...
	async getListCompare(
		...
		const { data } = await this.axios({
			...
			url:
				ApiUrl.baseUrl +
				ApiUrl.CALCULATION_GET_LIST_COMPARE_FOAM.replace("{pid}", pid).replace(
					"{uid}",
					uid
				),
			params: {
...
const ApiActFoamCalculation = new CalculationFoamApi();

export default ApiActFoamCalculation;
